...
% rootpath = '/mnt/data/';
% datapath = 'AZ051/2018-12-03_15-13-21/';
% localpath = '/mnt/data/Sorted/AZ051/2018-12-03_15-13-21/'
...
ops.fbinary             = fullfile(localpath,'mybinaryfile.dat'); % will be created for 'openEphys'		
ops.fproc               = fullfile(localpath,'temp_wh.dat'); % residual from RAM of preprocessed data		
ops.root                = fullfile(rootpath,datapath); % 'openEphys' only: where raw files are		
...
ops.chanMap             = fullfile(localpath,'chanMap.mat'); % make this file using createChannelMapFile.m		
ops.criterionNoiseChannels = 0.2; %1.001; % fraction of "noise" templates allowed to span all channel groups (see createChannelMapFile for more info). 		
...
ops.Th               = [6 12 12]; %[4 10 10];    % threshold for detecting spikes on template-filtered data ([6 12 12])		
ops.lam              = [10 30 30]; %[5 20 20];   % large means amplitudes are forced around the mean ([10 30 30])		
...
ops.initialize      = 'fromData'; %'no'; %'no'; %'fromData' or 'no'		
