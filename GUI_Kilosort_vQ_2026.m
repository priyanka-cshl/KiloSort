function varargout = GUI_Kilosort_vQ_2026(varargin)
% GUI_KILOSORT_VQ_2026 MATLAB code for GUI_Kilosort_vQ_2026.fig
%      GUI_KILOSORT_VQ_2026, by itself, creates a new GUI_KILOSORT_VQ_2026 or raises the existing
%      singleton*.
%
%      H = GUI_KILOSORT_VQ_2026 returns the handle to a new GUI_KILOSORT_VQ_2026 or the handle to
%      the existing singleton*.
%
%      GUI_KILOSORT_VQ_2026('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_KILOSORT_VQ_2026.M with the given input arguments.
%
%      GUI_KILOSORT_VQ_2026('Property','Value',...) creates a new GUI_KILOSORT_VQ_2026 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_Kilosort_vQ_2026_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_Kilosort_vQ_2026_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_Kilosort_vQ_2026

% Last Modified by GUIDE v2.5 05-May-2026 09:19:38

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_Kilosort_vQ_2026_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_Kilosort_vQ_2026_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_Kilosort_vQ_2026 is made visible.
function GUI_Kilosort_vQ_2026_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_Kilosort_vQ_2026 (see VARARGIN)

% Choose default command line output for GUI_Kilosort_vQ_2026
handles.output = hObject;

% Load User specific defaults 
if ~isempty(varargin)
    handles = LoadKilosortDefaults2026(handles, varargin{1});
end
...
% UIWAIT makes GUI_Kilosort_vQ_2026 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_Kilosort_vQ_2026_OutputFcn(hObject, eventdata, handles) 
...
for i = 1:length(handles.db)   % for each session
    ...
    for j = 1:uniquesessions % for each file within the session
        ...
        ops.Nchan = numel(ops.ActiveChannels); % number of active channels - channels that will be in the binary file that gets used for sorting
        ops.Nchanbinary = handles.recording_settings.Data(1); % total channels in the binary file that was written by OEPS - if data format is flat binary
        ops.NchanAux = handles.recording_settings.Data(2);
        %ops.NchanTOT = ops.Nchan;  % we don't use this
        
        % create a list of valid channels - account for the unloaded channels
        ops.DeadChans = eval(handles.NoisyChannels.String);
        ...
        if strcmp(eventdata.Source.Tag, 'make_config_file')
         ...
         % for the binary file
        [binarypath, ~, ~] = fileparts(ops.fbinary);

        sorted = 0;
        if ~handles.Overwrite.Value
        ...
        if ~sorted
            ...
            if ~ops.onlybinary
                ...
                disp(['making binary file: ',fullfile(rootpath,datapath)]);
            end
            master_file_Albeanu_VQ_2026;
            % change permissions
